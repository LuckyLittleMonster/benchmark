/*
 * GPU-to-GPU Bandwidth Test with GPU Direct DISABLED
 * 
 * This program measures bandwidth between two GPUs WITHOUT using GPU Direct.
 * It uses fork() to create two processes and CUDA IPC to share memory handles.
 * 
 * To ensure GPU Direct is disabled, you can:
 * 1. Set environment variable: export CUDA_VISIBLE_DEVICES_PEER_ACCESS=0
 * 2. Or use: export CUDA_DEVICE_ORDER=PCI_BUS_ID (to disable default peer access)
 * 
 * Key features:
 * - Uses CUDA IPC (cudaIpcMemHandle) to access remote GPU memory
 * - Explicitly disables peer access if it was enabled
 * - Data path: GPU 0 -> System Memory -> GPU 1 (not direct GPU-to-GPU)
 * - Uses CUDA kernels to access remote GPU memory (triggers implicit copies)
 */
...
void parent_process(SharedData* shared) {
    ...
    // Explicitly disable peer access if it was auto-enabled
    // This forces all data to go through system memory
    cudaError_t peer_err = cudaDeviceDisablePeerAccess(1);
    if (peer_err == cudaSuccess) {
        cout << "Parent: Successfully disabled peer access (was auto-enabled)" << endl;
    } else if (peer_err == cudaErrorPeerAccessNotEnabled) {
        cout << "Parent: Peer access was not enabled (good - GPU Direct is disabled)" << endl;
        cudaGetLastError(); // Clear the error
    } else {
        CUDA_CHECK(peer_err);
    }
    
    cout << "\n=== P2P Bandwidth Test (GPU 0 -> GPU 1, GPU Direct DISABLED) ===" << endl;
    cout << "Data path: GPU 0 -> System Memory -> GPU 1" << endl;
    cout << ljust("size", 30) << "\t" 
         << ljust("time(ms)", 30) << "\t" 
